...
class Oauth2Subscriber implements SubscriberInterface
{
    ...
    /** @var callable|null */
    protected $invalidRefreshTokenCallback;
        ...
        if ($this->refreshTokenGrantType) {
            ...
            if ($this->refreshTokenGrantType->hasRefreshToken()) {
                try {
                    ...
                    if ($e->getResponse()->getStatusCode() === 400 && isset($this->invalidRefreshTokenCallback)) {
                        $accessToken = call_user_func($this->invalidRefreshTokenCallback, $e);
                        if ($accessToken) {
                            return $accessToken;
                        }
                    }
    ...
    /**
     * Set a callback that will react to an invalid refresh token error.
     *
     * @param callable $callback
     *   A callback which accepts one argument, the BadResponseException, and
     *   returns an AccessToken or null.
     */
    public function setInvalidRefreshTokenCallback(callable $callback)
    {
      $this->invalidRefreshTokenCallback = $callback;
    }
}
